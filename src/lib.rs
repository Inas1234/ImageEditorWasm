...
pub fn blur_image(image_data: &[u8], width: u32, height: u32, sigma: f32) -> Vec<u8> {
    ...
    let mut buf = Vec::new();
    let _ = blurred_img.write_to(&mut buf, ImageOutputFormat::Png);
...
pub fn resize_img(image_data: &[u8], width: u32, height: u32, new_width: u32, new_height: u32) -> Vec<u8> {
    ...
    let mut buf = Vec::new();
    let _ = resized_img.write_to(&mut buf, ImageOutputFormat::Png);
...
pub fn grayscale_img(image_data: &[u8], width: u32, height: u32) -> Vec<u8> {
    ...
    let mut buf = Vec::new();
    let _ = grayscale_img.write_to(&mut buf, ImageOutputFormat::Png);
